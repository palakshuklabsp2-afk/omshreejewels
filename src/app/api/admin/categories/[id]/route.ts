...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const body: { name?: string; slug?: string; image?: string; isActive?: boolean; sortOrder?: number } = {};
  if (typeof raw.name === "string") {
    body.name = raw.name;
    body.slug = slugify(raw.name);
  }
  if (typeof raw.image === "string") body.image = raw.image;
