...
import { useCart, cartLineKey } from "@/components/cart-provider";
    ...
    <div className="mx-auto max-w-4xl px-4 py-10">
      ...
      {items.length === 0 ? (
        ...
        <div className="mt-6 space-y-4">
          {items.map((item) => (
            <div key={cartLineKey(item)} className="flex gap-4 rounded-2xl bg-white border p-3">
              <div className="relative h-24 w-24 rounded-xl overflow-hidden">
                <Image src={item.image} alt={item.name} fill className="object-cover" />
              </div>
              <div className="flex-1">
                <div className="font-medium">{item.name}</div>
                {item.size ? <div className="text-xs text-zinc-500">Size: {item.size}</div> : null}
                <div className="text-crimson text-sm">{formatInr(item.price)}</div>
                <div className="mt-2 flex items-center gap-2">
                  <button className="h-8 w-8 rounded-full border" onClick={() => setQty(cartLineKey(item), item.qty - 1)}>
                    −
                  </button>
                  <span>{item.qty}</span>
                  <button className="h-8 w-8 rounded-full border" onClick={() => setQty(cartLineKey(item), item.qty + 1)}>
                  ...
                  <button className="ml-3 text-sm text-red-600" onClick={() => remove(cartLineKey(item))}>
                    Remove
                  </button>
                </div>
              </div>
